...
// 	$usuario = stripslashes($usuario);
	...
	if (empty($usuario) or empty($password) or empty($password2)) {
		...
		$statement = $conexion->prepare('SELECT * FROM user WHERE usuario = :usuario LIMIT 1');
	...
	if ($errores == '') {
		$statement = $conexion->prepare('INSERT INTO user (id, usuario, pass) VALUES (null, :usuario, :pass)');
